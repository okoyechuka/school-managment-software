<?php
/*
	#	2co Payment module callback for SOA
	#	location: gateway/2checkout/callback.php
...
if($Transaction_ref!="") {
	...
	$is_valid = 0;
	$gtpay_merchant_id = trim(paymentGatewayData($Payment_gateway,'param1'));	//This is saved in param1 column
	$gtpay_hash_key = trim(paymentGatewayData($Payment_gateway,'param2'));	//This is saved in param2 column	

	if(isset($_POST['gtpay_tranx_id']) && ($_POST['gtpay_tranx_status_code']!='00'))	{
		$is_valid = ($_POST['gtpay_tranx_status_code']!='00')?-1:1;
		$comments = $_POST['gtpay_tranx_status_msg'];
		$amount = $_POST['gtpay_tranx_amt'];
	} else 	{
		$hash=hash("sha512",$gtpay_merchant_id.$Transaction_ref.$gtpay_hash_key);
		$url="https://ibank.gtbank.com/GTPayService/gettransactionstatus.json?mertid=$gtpay_merchant_id&amount=$gtpay_amount&tranxid=$Transaction_ref&hash=$hash";
		...
		if($response_code == 200) { 
			...
			$comments="HTTP Error $response_code: $response. ";
		}
		
		if(!empty($json)) {		
			$is_valid = ($json['ResponseCode']=='00')?1:-1;
			$comments = $json['ResponseDescription'];
		}else { 
			$is_valid = -1;
			$comments = 'Payment Failed! '.$response;
		}
	}
}
