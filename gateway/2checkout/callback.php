...
if($Transaction_ref!="") {
	...
	$key = strtoupper(md5(trim(paymentGatewayData($Payment_gateway,'param2')) . $vendorNumber . $orderNumber . $orderTotal));						
	if(floatval($orderTotal) <=0 ) {
		$is_valid = 0;
		$comments = "Incorrect amount paid. ";
	} elseif ($vendorNumber != trim(paymentGatewayData($Payment_gateway,'param1'))) {
		$is_valid = 0;
		$comments = "The payment was made into a different 2checkout account.";
	} elseif($ipnData["key"] == $key || $ipnData["x_MD5_Hash"] == $key)	{
		$is_valid = 1;
		$comments = 'Transaction Successful';
	} else { 
		$comments = "Verification failed! Invalid payment hash";
	}
}
