...
if(isset($_GET['keyword'])){
	...
	foreach($ser as $look)	{
	    ...
	    if (!empty($term))	    {
	         $clauses[] = "a.title LIKE '%$term%' OR a.description LIKE '%$term%'";
	    }	    else	    {
	...
	$sql=$query = "select a.* FROM e_courses a JOIN class_course b ON a.id = b.course_id WHERE b.class_id = '$student_class' AND a.school_id = '$school_id' AND a.start_date <= '$today' AND $filter ";
	...
	$sql=$query = "SELECT a.* FROM e_courses a JOIN class_course b ON a.id = b.course_id WHERE b.class_id = '$student_class' AND a.school_id = '$school_id' AND a.start_date <= '$today' ORDER BY a.id DESC LIMIT $pageLimit,$setLimit";
	$result = mysqli_query($server, $query) or die(mysqli_error($server));
	$num = mysqli_num_rows($result);
	if($num < "1")		{
...
if(isset($_REQUEST['no_pay'])) {
	$message = "You have not paid the subscription fee for the selected online course. <a href='userfee?pay=".$student_id."&student_id=<".$student_id."&fee_id=".$_REQUEST['no_pay']."&session_id=".$session."&term_id=".$term."&class_id=".$student_class."'>Click Here</a> to pay the fee now";
	$class="blue";
}
?>
        ...
        <div class="panel-body">
        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr class="inner">
                ...
                <td valign="middle">
                <?php if(empty($row['expire_date']) || strtotime($row['expire_date']) > time()) {?>
                <a href="usercourses?view=<?php echo $id;?>"><button>Course Details</button></a>
                <a href="usercoursecontent?course=<?php echo $id;?>"><button class="success">View Lessons</button></a>
                <?php } ?>
                </td>
              </tr>
              <?php	$i++; } ?>
