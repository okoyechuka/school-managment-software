...
$searchword = filterinp($_GET['keyword']);
	...
	foreach($ser as $look)
	{
	    ...
	    if (!empty($term))
	    {
	         $clauses[] = "p.buyer LIKE '%$term%' OR p.date LIKE '%$term%' OR p.added_by LIKE '%$term%'";
	    }
	    else
	    {
	         $clauses[] = "p.date LIKE '%%' OR p.id LIKE '%%' OR p.buyer LIKE '%%' ";
	    }
	}
	...
	if($category > 0) {
		$sql=$query = "select * FROM sales p WHERE p.school_id = '$school_id' AND stock_id = '$stock_id' AND $filter ";
	}
	...
	$sql=$query = "SELECT * FROM sales WHERE school_id = '$school_id' AND stock_id = '$stock_id' ORDER BY id DESC LIMIT $pageLimit,$setLimit";
	$result = mysqli_query($server, $query);
	$num = mysqli_num_rows($result);
	if($num < "1")
		{
		$message = "There are currently no sales records to display for this item!";
		$class="blue";
		}
}
...
<div class="wrapper">
	...
	<div id="search-pan">
        ...
        <input type="search" name="keyword" placeholder="Search Sales" />
        ...
        <a href="admin/store?addSale"><button type="button" class="submit success">Add Sales</button></a>
        <?php } ?>
        </form>
    </div>
    <h3>Sales Records for <?php echo itemName(filterinp($_GET['stock_id'])); ?></h3>
	<div class="panel" style="border-color: transparent;">
        <div class="panel-body">
        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <th width="15%" scope="col">Date</th>
                <th width="20%" scope="col">Buyer</th>
                <th width="15%" scope="col">Unit Price</th>
                <th width="15%" scope="col">Quantity</th>
                <th width="15%" scope="col">Total</th>
                <th width="15%" scope="col">Sold By</th>
              </tr>
               <?php
